        ...
        match (&self.state, cmd) {
            ...
            (BlockProtocolState::Idle, HostToLedgerCmd::Start) => {
                ...
                for chunk in payload.chunks_exact(HASH_LEN) {
                    let hash: Hash = chunk.try_into().map_err(|_| StatusWords::BadLen)?;
                    self.input_hashes
                        .try_push(hash)
            ...
            (BlockProtocolState::WaitingChunk { .. }, HostToLedgerCmd::GetChunkResponseFailure) => {
                Err(Reply(0x6A88)) // Referenced data not found
            }

            // PUT_CHUNK_RESPONSE: Host acknowledged storing data
            (BlockProtocolState::WaitingPutResponse, HostToLedgerCmd::PutChunkResponse) => {
